...
// 6. Razorpay Flow integration (replace the fetch pay with Razorpay conditional)
              ...
              return (
                <div className={\`flex flex-col gap-1 text-[10px] px-2 py-1.5 border rounded-lg mt-2 \${color}\`}>
                  <div className="flex justify-between font-bold">
                    ...
                    <span className="px-1.5 py-0.5 rounded bg-white/50">{anyOccupied ? 'Occupied' : 'Available'}</span>
                  </div>
...
content = content.replace(
  ...
  `                          <method.icon className={\`w-4 h-4 \${isSelected ? 'text-white' : 'text-slate-500'}\`} />\n                          {method.label}`
);

fs.writeFileSync(path, content, 'utf8');
console.log('Patched POSView.tsx successfully.');
