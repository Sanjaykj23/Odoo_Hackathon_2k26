...
// 3. Razorpay Load
content = content.replace(
  /\/\/ Initialize POS config and WebSockets\s+useEffect\(\(\) => \{\s+if \(\!token\) return;/,
  `  // Initialize POS config and WebSockets\n  useEffect(() => {\n    const script = document.createElement('script');\n    script.src = 'https://checkout.razorpay.com/v1/checkout.js';\n    script.async = true;\n    document.body.appendChild(script);\n\n    if (!token) return;`
);

// 4. Kitchen Dispatch & Payment Check
content = content.replaceAll(/if \(\!selectedTableId\) \{/g, 'if (selectedTableIds.length === 0) {');

content = content.replace(
  /onSendToKitchen\(cart, resolvedCustomer, appliedPromo, orderNotes, selectedTableId\);/g,
  `onSendToKitchen(cart, resolvedCustomer, appliedPromo, orderNotes, selectedTableIds.join(','));`
);

// 5. Clear States
content = content.replaceAll(/setSelectedTableId\(''\);/g, `setSelectedTableIds([]);`);

content = content.replace(
  /setCustomerName\(''\);\n\s*setAppliedPromo\(null\);\n\s*setOrderNotes\(''\);/g,
  `setCustomerName('');\n    setCustomerPhone('');\n    setAppliedPromo(null);\n    setOrderNotes('');`
);

// 6. Payment Dispatch
content = content.replace(
  /if \(cart\.length === 0 \|\| \!token \|\| \!activeSession \|\| \!selectedTableId\) return;/g,
  `if (cart.length === 0 || !token || !activeSession || selectedTableIds.length === 0) return;`
);
content = content.replace(
  /table_id: selectedTableId,/g,
  `table_id: selectedTableIds.join(','),\n      guest_count: guestCount,`
);

// 7. Razorpay Flow integration
const payFlowMatch = /      \/\/ 2\. Submit payment to record transaction and mark order Paid\s*const payResponse = await fetch\(`\/api\/orders\/\$\{orderData\.id\}\/pay`, \{\s*method: 'POST',\s*headers: \{\s*'Content-Type': 'application\/json',\s*'Authorization': `Bearer \$\{token\}`\s*\},\s*body: JSON\.stringify\(\{\s*payment_method: paymentMethod,\s*transaction_ref: transactionRef,\s*amount: totals\.total\s*\}\)\s*\}\);\s*const payData = await payResponse\.json\(\);\s*if \(\!payResponse\.ok\) \{\s*throw new Error\(payData\.error \|\| 'Failed to process payment\.'\);\s*\}/g;
...
content = content.replace(payFlowMatch, rzpReplacement);

content = content.replace(
  /setSelectedTableId\(tableId\);/g,
  `if (!selectedTableIds.includes(tableId)) setSelectedTableIds([...selectedTableIds, tableId]);`
);

// Grid rendering
content = content.replace(
  /Occupied: \{ border: 'border-rose-200 hover:border-rose-300', bg: 'bg-rose-50\/30', badge: 'bg-rose-50 text-rose-700 border-rose-200', dot: 'bg-rose-500 animate-pulse' \},/g,
...
content = content.replace(
  /const isSelected = selectedTableId === table\.id;/g,
  `const isSelected = selectedTableIds.includes(table.id);`
);
content = content.replace(
  /<option value="Occupied">Occupied<\/option>\s*<option value="Reserved">Reserved<\/option>/g,
  `<option value="Occupied">Occupied</option>\n                                  <option value="Partially Occupied">Partially Occupied</option>\n                                  <option value="Reserved">Reserved</option>`
);

// Customer Input Row
content = content.replace(
  /\{\/\* Customer Name Input \*\/\}\s*<div className="relative pb-3 border-b border-\[\#e2e8f0\]">\s*<div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none mb-3">\s*<User className="w-3 h-3 text-slate-400" \/>\s*<\/div>\s*<input\s*type="text"\s*placeholder="Walk-in Customer Name \(Optional\)"\s*value=\{customerName\}\s*onChange=\{\(e\) => setCustomerName\(e\.target\.value\)\}\s*className="w-full pl-8 pr-3 py-2 text-xs bg-white border border-\[\#e2e8f0\] rounded-xl focus:outline-none focus:ring-1 focus:ring-purple-500 focus:border-purple-500 text-slate-800 transition-shadow"\s*\/>\s*<\/div>/g,
...
content = content.replace(
  /\{selectedTableId && \(\s*<span className="inline-block mt-2 px-2 py-0\.5 bg-white\/10 rounded text-\[10px\] font-semibold text-slate-300">\s*Table: \{tables\.find\(t => t\.id === selectedTableId\)\?\.number\}\s*<\/span>\s*\)\}/g,
...
content = content.replace(
  /\{\s*id: 'Cash',\s*label: 'Cash'\s*\},\s*\{\s*id: 'Card',\s*label: 'Card Term'\s*\},\s*\{\s*id: 'UPI',\s*label: 'UPI \/ App'\s*\}/g,
...
content = content.replace(
  /\{method\.id === 'Cash' && <Banknote className=\{`w-4 h-4 \$\{isSelected \? 'text-white' : 'text-slate-500'\}`\} \/>\}\s*\{method\.id === 'Card' && <CreditCard className=\{`w-4 h-4 \$\{isSelected \? 'text-white' : 'text-slate-500'\}`\} \/>\}\s*\{method\.id === 'UPI' && <Smartphone className=\{`w-4 h-4 \$\{isSelected \? 'text-white' : 'text-slate-500'\}`\} \/>\}\s*\{method\.label\}/g,
